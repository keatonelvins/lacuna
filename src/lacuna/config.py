...
class SchedulerConfig(BaseModel):
    ...
    min_lr_ratio: float = Field(0.1, ge=0, le=1, description="Minimum LR as ratio of max LR")


class DataConfig(BaseModel):
    """Base config for data loading"""
    batch_size: int = Field(32, ge=1, description="Training batch size")
    micro_batch_size: int = Field(8, ge=1, description="Micro batch size for gradient accumulation")
...
class SFTDataConfig(DataConfig):
    """SFT data config"""
    dataset_name: str = Field("trl-internal-testing/dolly-chatml-sft", description="HuggingFace dataset name")
...
class CheckpointConfig(BaseModel):
    ...
    save_dir: Path = Field(Path("checkpoints"), description="Directory to save checkpoints")
